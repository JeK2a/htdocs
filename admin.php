...
if($login&&$login['admin']) {
	...
	print "<anchor>".$lang['ok']."<go href=\"$PHP_SELF?act=update&amp;id=$id&amp;pass=$pass&amp;mod=$mod\" method=\"post\"><postfield name=\"name\" value=\"$(name)\"/><postfield name=\"pos\" value=\"$(pos)\"/></go></anchor><br/>";
	} else {
	...
	if(@mysql_query("insert into `".$px.$stable."` values(0, 'room', 'room".$i."', '$name', '$name', '$pos');")) print "<b>Операция успешно завершена!</b><br/>";
	}
	break;
	//редактируем ник
	case 'editnik':
	...
	print "<a href=\"$PHP_SELF?id=$id&amp;pass=$pass&amp;mod=optimize\">".$lang['optimize']."</a></small><br/>";
	break;
	}
